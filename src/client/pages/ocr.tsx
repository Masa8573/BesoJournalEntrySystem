  ...
  const processOneDocument = async (result: OCRResultItem) => {
    ...
    try {
      ...
      if (urlError || !signedUrlData?.signedUrl) {
        throw new Error(`ファイルURLの取得に失敗: ${urlError?.message || '署名URL生成エラー'}`);
      }
      ...
      if (dbSaveError || !savedEntry) {
        throw new Error(`仕訳ヘッダー保存エラー: ${dbSaveError?.message || '保存失敗'}`);
      }
      ...
      if (journalEntry.lines && journalEntry.lines.length > 0) {
        ...
        const { error: linesError } = await supabase
          .from('journal_entry_lines').insert(linesToInsert);

        if (linesError) {
          // ヘッダーはあるが明細がない不整合を防ぐ：ヘッダーを削除して再throw
          await supabase.from('journal_entries').delete().eq('id', savedEntry.id);
          throw new Error(`仕訳明細行の保存エラー: ${linesError.message}`);
        }
      }
