...
export default function TaxCategoriesPage() {
  ...
  const loadClientSettings = async (clientId: string) => {
    try {
      const { data } = await supabase.from('client_tax_category_settings' as any).select('*').eq('client_id', clientId);
  ...
  const handleDeleteRate = async (rate: TaxRate) => {
    ...
    const { error } = await supabase.from('tax_rates').delete().eq('id', rate.id);
    if (error) alert('削除に失敗しました: ' + error.message);
    else loadData();
  };

  const handleToggleRateCurrent = async (rate: TaxRate) => {
    await supabase.from('tax_rates').update({ is_current: !rate.is_current }).eq('id', rate.id);
    loadData();
  };
