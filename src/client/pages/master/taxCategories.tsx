...
export default function TaxCategoriesPage() {
  ...
  const loadData = async () => {
    ...
    if (taxRes.error) {
      console.error('税区分取得エラー:', taxRes.error.message);
      alert('税区分データの取得に失敗しました: ' + taxRes.error.message);
    } else if (taxRes.data) {
      setTaxCategories(taxRes.data as TaxCategory[]);
    }

    if (clientRes.error) {
      console.error('顧客取得エラー:', clientRes.error.message);
    ...
    <div className="space-y-6">
      ...
      <div className="card">
        <h2 className="text-sm font-medium text-gray-700 mb-4">
          {selectedClientName ? `${selectedClientName}さんの税区分設定` : '税区分設定'}
        </h2>

        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            <div className="text-2xl font-bold text-gray-900 mb-1">{clientSettings.totalCount}</div>
            <div className="text-xs text-gray-600">税区分数</div>
          </div>
          <div className="text-center p-4 bg-green-50 rounded-lg">
            <div className="text-2xl font-bold text-green-600 mb-1">{clientSettings.defaultCount}</div>
            <div className="text-xs text-gray-600">デフォルト使用</div>
          </div>
          <div className="text-center p-4 bg-blue-50 rounded-lg">
            <div className="text-2xl font-bold text-blue-600 mb-1">{clientSettings.incomeCount}</div>
            <div className="text-xs text-gray-600">収入用</div>
          </div>
          <div className="text-center p-4 bg-red-50 rounded-lg">
            <div className="text-2xl font-bold text-red-600 mb-1">{clientSettings.expenseCount}</div>
            <div className="text-xs text-gray-600">支出用</div>
          </div>
          <div className="text-center p-4 bg-purple-50 rounded-lg">
            <div className="text-2xl font-bold text-purple-600 mb-1">{clientSettings.sharedCount}</div>
              ...
              {filteredCategories.length === 0 ? (
                ...
                filteredCategories.map((category) => (
                  <tr key={category.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-xs text-gray-500 font-mono">{category.code}</td>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600">{getDirectionLabel(category)}</td>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <ul className="text-sm text-blue-800 space-y-1 list-disc list-inside">
          <li><strong>デフォルト:</strong> オンにすると、この税区分がfreee上でデフォルトで選択されます</li>
          <li><strong>収入:</strong> 売上方向の取引（direction=売上/共通）で使用可能な税区分</li>
          <li><strong>支出:</strong> 仕入方向の取引（direction=仕入/共通）で使用可能な税区分</li>
