...
export default function ItemsPage() {
  ...
  const loadItems = async () => {
    ...
    const { data, error } = await supabase
      ...
      .order('name');
    if (error) console.error('品目取得エラー:', error.message);
  ...
  const handleAddAlias = async () => {
    ...
    const { error } = await supabase.from('item_aliases').insert([{
  ...
  const handleDeleteAlias = async (alias: ItemAlias) => {
    ...
    const { error } = await supabase.from('item_aliases').delete().eq('id', alias.id);
    if (error) alert('削除に失敗しました: ' + error.message);
