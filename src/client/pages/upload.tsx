...
export default function UploadPage() {
  ...
  const uploadToSupabase = async (uploadFile: UploadedFile) => {
    ...
    if (dbError || !docRecord) { await supabase.storage.from('documents').remove([storagePath]); throw new Error(`DB save failed: ${dbError}`); }
  ...
  const removeFile = async (fileId: string) => {
    ...
    if (!file) return;
    if (file.storagePath) await supabase.storage.from('documents').remove([file.storagePath]);
